...
const socket = useRef(io('http://fe80::8f48:a666:fd96:6627%3')).current; // replace with LAN IP or use 10.0.2.2 for Android emulator
...
const AccelerometerGraph: React.FC = () => {
  ...
  const processBuffer = () => {
    ...
    //----- added for server ( delete if not needed)--------------------------------------------------------------------
    // Send the last known Z displacement to server
    const lastZ = positionBatch[positionBatch.length - 1]?.z || 0;
  ...
  useEffect(() => {
    ...
    //;
    // added for server-------------------------------------------------
    socket.on('eventDetected', (event:any) => {
      Alert.alert('Event Detected', event.type);
  });
  return () => {
    socket.off('eventDetected');
  };
}, []);
